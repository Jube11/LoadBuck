...
function Calculator({ user }) {
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          title: 'LoadBuck Trip Calculation',
          text: `Trip: ${formData.origin} → ${formData.destination}\nProfit: $${result.total_profit}`,
          url: window.location.href
        })
      } else {
        // Fallback to clipboard
        const text = `LoadBuck Calculation:\n` +
          `Route: ${formData.origin} → ${formData.destination}\n` +
          `Rate: $${formData.rate_offered}\n` +
          `Miles: ${result.total_miles}\n` +
          `Profit: $${result.total_profit} ($${result.profit_per_mile}/mi)\n` +
  ...
  const exportToPDF = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>LoadBuck - Trip Calculation</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; }
            h1 { color: #1a237e; border-bottom: 3px solid #ff6d00; padding-bottom: 10px; }
            .route { font-size: 1.2em; color: #666; margin-bottom: 20px; }
            .result-box { background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0; }
            .profit { font-size: 2em; color: ${result.recommendation === 'YES' ? '#00c853' : result.recommendation === 'NO' ? '#ff1744' : '#ff9100'}; font-weight: bold; }
            .recommendation { display: inline-block; padding: 5px 15px; border-radius: 20px; color: white; background: ${result.recommendation === 'YES' ? '#00c853' : result.recommendation === 'NO' ? '#ff1744' : '#ff9100'}; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            td { padding: 10px; border-bottom: 1px solid #ddd; }
            .label { color: #666; }
            .value { text-align: right; font-weight: bold; }
            .total { background: #e8eaf6; font-size: 1.1em; }
            .footer { margin-top: 40px; text-align: center; color: #999; font-size: 0.9em; }
          </style>
        </head>
        <body>
          <h1>LoadBuck</h1>
          <p class="route">${formData.origin || 'Origin'} → ${formData.destination || 'Destination'}</p>
          
          <div class="result-box">
            <span class="recommendation">${result.recommendation}</span>
            <div class="profit">$${result.total_profit.toLocaleString()}</div>
            <div>$${result.profit_per_mile}/mile</div>
          </div>
          
          <table>
            <tr><td class="label">Rate Offered</td><td class="value">+$${parseFloat(formData.rate_offered).toLocaleString()}</td></tr>
            <tr><td class="label">Total Miles</td><td class="value">${result.total_miles.toLocaleString()} mi</td></tr>
            <tr><td class="label">Fuel Cost</td><td class="value">-$${result.fuel_cost.toLocaleString()}</td></tr>
            <tr><td class="label">Tolls</td><td class="value">-$${parseFloat(formData.tolls || 0).toLocaleString()}</td></tr>
            <tr><td class="label">Maintenance</td><td class="value">-$${result.maintenance_cost.toLocaleString()}</td></tr>
            <tr class="total"><td class="label">YOUR PROFIT</td><td class="value">$${result.total_profit.toLocaleString()}</td></tr>
          </table>
          
          <div class="footer">
            <p>Generated by LoadBuck - Know your profit before you haul</p>
          ...
          <div className="results-section animate-slide-up">
            <div className="quick-stats">
              <div className="stat-item">
                <span className="stat-value">${result.profit_per_mile}</span>
                <span className="stat-label">per mile</span>
              </div>
              <div className="stat-item">
                <span className="stat-value">${(result.total_profit / result.total_miles * 100).toFixed(0)}</span>
                <span className="stat-label">profit %</span>
              </div>
              <div className="stat-item">
                <span className="stat-value">${(parseFloat(formData.rate_offered) / result.total_miles).toFixed(2)}</span>
                <span className="stat-label">rate/mi</span>
              </div>
            </div>
